...
def gibbs_sampling(x):
    ...
    print ("intial chain r1: "+str(lamda1))
    print ("intial chain r2: "+str(lamda2))
    # Store the samples
    chain_n      =torch.zeros(E)
    chain_lambda1=torch.zeros(E)
    chain_lambda2=torch.zeros(E)
    ...
    for e in range(E):
        if e%100 == 0:
            ...
            start_time = time.time()
        lambda1 = pyro.sample("lamda1", pyro.distributions.Gamma(a+sum(x[0:n]), (n.item()+b)))
        lambda2 = pyro.sample("lamda2", pyro.distributions.Gamma(a+sum(x[n:N]), (N-n.item()+b)))
        # sample n, Equation 10
        mult_n=torch.zeros(N,1)
        for i in range(N):
            mult_n[i]=sum(x[0:i])*torch.log(lambda1)-i*lambda1+sum(x[i:N])*torch.log(lambda2)-(N-i)*lambda2
        mult_n=torch.exp(mult_n-max(mult_n))
        weights = torch.div(mult_n, sum(mult_n))
        out = torch.multinomial(weights.T,1,replacement=True)
        n=out
...
chain_n =chain_n[BURN_IN:E].numpy()
chain_lambda1=chain_lambda1[BURN_IN:E].numpy()
chain_lambda2=chain_lambda2[BURN_IN:E].numpy()
