...
def fit_EM(m1, m2, filename, data, image):
    ...
    z1 = np.asarray([[13, 20, 29], [13, 23, 37], [13, 23, 29.0]])
    cov1 = np.cov(z1)        #covariance 1
    z2 = np.asarray([[9, -58, 7], [8, -7, 10], [6, -4, 6.0]])
    cov2 = np.cov(z2)        #covariance 2
    ...
    while (1):
        ...
        for y in image:
            for x in y:
                probs2 = torch.distributions.LowRankMultivariateNormal(torch.tensor(mean1), torch.tensor(cov1).float()).log_prob(torch.tensor(x))
                #probs1 = dist.DirichletMultinomial(torch.tensor(mean1), torch.tensor(cov1).float()).log_prob(torch.tensor(x))
                #test_pro1 = torch.exp(probs1)
